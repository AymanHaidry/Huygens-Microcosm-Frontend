/* ─── Nanofossil Frontend v2.1 ───
   Supabase Auth + DB + GitHub Actions
   FIXED: Auth gate now visible by default, hidden only on confirmed login
*/
...
const CONFIG = {
  ...
  authCheckTimeout: 8000  // max time to wait for auth check before showing gate
};
...
async function checkAuth() {
  // Set a safety timeout: if auth check takes too long, show the gate
  const authTimeout = setTimeout(() => {
    console.warn('Auth check timed out — showing login gate');
    showAuthGate();
  }, CONFIG.authCheckTimeout);
  ...
  try {
    ...
    // Safer session check — avoid destructuring null
...
function showApp() {
  ...
  console.log('App shown, user:', currentUser?.email);
}
    ...
    return `
      <button class="recent-item" data-job-id="${item.id}" data-run-id="${item.github_run_id || ''}">
        ...
        <span class="recent-item-text">
          ...
          <div class="recent-item-meta">${date} · ${item.status}</div>
        </span>
...
function renderReport(data) {
  ...
  let html = `
    ...
    <div class="report-meta">
      <span>Nanofossil</span><span>·</span><span>${date}</span><span>·</span><span>${r.sources ? r.sources.length : 0} sources</span>
    </div>
  `;
  ...
  if (r.comparison && r.comparison.headers && r.comparison.rows) {
    html += `<h2 class="section-heading">Comparison</h2><table class="comparison-table"><thead><tr>`;
  ...
  if (r.confidence_notes) {
    html += `
      <div class="confidence-note">
        <svg class="confidence-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
    ...
    html += `
      <div class="sources-section">
        <button class="sources-toggle" onclick="this.nextElementSibling.classList.toggle('open')">
          <span>Sources</span>
          <span class="sources-count">${r.sources.length} total · ${primaryCount} primary · ${secondaryCount} secondary</span>
        </button>
      ...
      html += `
        <li class="source-item">
          ...
          <div class="source-content">
            ...
            <a class="source-url" href="${escapeHtml(source.url)}" target="_blank" rel="noopener">${escapeHtml(source.url)}</a>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (sbConfig.url && sbConfig.key) {
    ...
    if (inited) {
      ...
      // Supabase failed to init — still show auth gate so user can configure
      showAuthGate();
    }
  } else {
    showAuthGate();
